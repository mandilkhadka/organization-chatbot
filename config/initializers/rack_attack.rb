...
module Rack
  class Attack
    ...
    throttle("admin_login/email", limit: 5, period: 5.minutes) do |req|
      req.params["email"].to_s.downcase.gsub(/\s+/, "").presence if req.path == "/admin/login" && req.post?
    end
    ...
    throttle("document_upload/user", limit: 20, period: 1.hour) do |req|
      req.env["warden"]&.user&.id if req.path == "/admin/documents" && req.post?
    end

    # Throttle bulk uploads more strictly
    throttle("bulk_upload/user", limit: 5, period: 1.hour) do |req|
      req.env["warden"]&.user&.id if req.path == "/admin/documents/bulk_create" && req.post?
    end
